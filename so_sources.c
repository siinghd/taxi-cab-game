...
int main(int argc, char const* argv[])
{
    ...
    while (1) {
        do {
            generateRowAndCol(&randrow, &randcol, SO_HEIGHT, SO_WIDTH);
        ...
        printf(ANSI_COLOR_MAGENTA "SO_SOURCE" ARROWRIGHT ANSI_COLOR_YELLOW "  (PID=%d): Q_id=%d: type=%ld |xSource= %d -  ySource= %d| " ARROWRIGHT "  |xDestination= %d -  yDestination= %d| SENT\n",
...
{
    if (signal == SIGALRM) {
        printf(ANSI_COLOR_MAGENTA "SO_SOURCE" ARROWRIGHT ANSI_COLOR_BLUE "  SIGALARM received  waking up and sending message...\n");
        alarm(getRandomNumberInRange(1, 5));
    }
    else if (signal == SIGILL) {
        printf(ANSI_COLOR_MAGENTA "SO_SOURCE" ARROWRIGHT ANSI_COLOR_BLUE "  SIGILL received  waking up and sending message...\n");
        alarm(getRandomNumberInRange(1, 5));
    }
}